...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex justify-between items-center">
            <h2 class="text-xl font-bold text-gray-800">Manage FAQ</h2>
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 text-gray-500 text-xs uppercase">
                    <tr>
                        ...
                        <th class="py-4 px-4">Question</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-4 px-4 font-bold text-gray-800 w-1/2">{{ $faq->question }}</td>
                        <td class="py-4 px-4">
                            <span class="px-2 py-1 bg-indigo-50 text-[#6C5DD3] rounded text-xs font-bold">{{ $faq->category }}</span>
                        </td>
                        <td class="py-4 px-4">
                            <span class="px-2 py-1 rounded text-xs font-bold {{ $faq->is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                                {{ $faq->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="openEditModal('{{ $faq->id }}', '{{ addslashes($faq->question) }}', '{{ addslashes(str_replace(array("\r", "\n"), ' ', $faq->answer)) }}', '{{ $faq->category }}', '{{ $faq->is_active }}')" class="px-3 py-1.5 bg-[#6C5DD3] text-white text-xs font-medium rounded hover:bg-[#5b4ec2]">Edit</button>
                                <button onclick="openDeleteModal('{{ $faq->id }}')" class="px-3 py-1.5 bg-red-500 text-white text-xs font-medium rounded hover:bg-red-700">Delete</button>
                            </div>
                        </td>
                    </tr>
                    @empty
                    <tr><td colspan="5" class="text-center py-8 text-gray-500">No FAQs found.</td></tr>
                    @endforelse
                </tbody>
            </table>
            <div class="px-6 py-4">{{ $faqs->links() }}</div>
        </div>
    </div>
    ...
    <div id="createModal" class="relative z-50 hidden" role="dialog" aria-modal="true"><div class="fixed inset-0 backdrop-brightness-50 backdrop-blur-sm transition-opacity"></div>
        <div class="fixed inset-0 z-10 overflow-y-auto flex items-center justify-center p-4">
            <div class="relative transform rounded-xl bg-white text-left shadow-xl w-full max-w-lg p-6 border border-gray-200">
                <h3 class="text-lg font-bold text-[#6C5DD3] mb-4 border-b pb-2">Add FAQ</h3>
                <form action="{{ route('faqs.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Question</label>
                        <input type="text" name="question" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Category</label>
                        <select name="category" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition">
                            <option value="General">General (Umum)</option>
                            <option value="Ticket">Ticket & Payment</option>
                            <option value="Creator">Event Creator</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Answer</label>
                        <textarea name="answer" rows="4" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none resize-none transition" required></textarea>
                    </div>
                    <div>
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="button" onclick="closeModal('createModal')" class="px-4 py-2 bg-gray-100 rounded-lg text-gray-600 hover:bg-gray-200 transition">Cancel</button>
    ...
    <div id="editModal" class="relative z-50 hidden" role="dialog" aria-modal="true"><div class="fixed inset-0 backdrop-brightness-50 backdrop-blur-sm transition-opacity"></div>
        <div class="fixed inset-0 z-10 overflow-y-auto flex items-center justify-center p-4">
            <div class="relative transform rounded-xl bg-white text-left shadow-xl w-full max-w-lg p-6 border border-gray-200">
                <h3 class="text-lg font-bold text-[#6C5DD3] mb-4 border-b pb-2">Edit FAQ</h3>
                <form id="editForm" method="POST" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Question</label>
                        <input type="text" name="question" id="edit_question" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Category</label>
                        <select name="category" id="edit_category" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Answer</label>
                        <textarea name="answer" id="edit_answer" rows="4" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none resize-none transition" required></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                        <select name="is_active" id="edit_is_active" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition">
                            <option value="1">Active</option>
                            <option value="0">Inactive</option>
                        </select>
                    </div>
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="button" onclick="closeModal('editModal')" class="px-4 py-2 bg-gray-100 rounded-lg text-gray-600 hover:bg-gray-200 transition">Cancel</button>
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto flex items-center justify-center p-4">
            <div class="relative transform rounded-xl bg-white text-left shadow-2xl w-full max-w-md p-6 border border-gray-200">
                <div class="text-center mt-2">
                    ...
                    <h3 class="text-xl font-bold text-[#6C5DD3] leading-snug px-4">Delete This FAQ?</h3>
    ...
    <script>
        ...
        function openEditModal(id, q, a, cat, active) {
            document.getElementById('edit_question').value = q;
            document.getElementById('edit_answer').value = a;
            document.getElementById('edit_category').value = cat;
            document.getElementById('edit_is_active').value = active;
            document.getElementById('editForm').action = '/admin/dashboard/faqs/' + id;
            document.getElementById('editModal').classList.remove('hidden');
        }
        
        function openDeleteModal(id) {
            document.getElementById('deleteForm').action = '/admin/dashboard/faqs/' + id;
