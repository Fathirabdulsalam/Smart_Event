...
@section('content')
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex justify-between items-center">
            <h2 class="text-xl font-bold text-gray-800">Master Kategori Tiket</h2>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-4 px-4">
                            <span class="px-3 py-1 rounded-full text-xs font-bold {{ $cat->is_paid ? 'bg-indigo-100 text-indigo-700' : 'bg-green-100 text-green-700' }}">
                                {{ $cat->is_paid ? 'Berbayar' : 'Gratis' }}
                            </span>
                        </td>
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="openEditModal('{{ $cat->id }}', '{{ $cat->name }}', '{{ $cat->is_paid }}')" class="flex items-center gap-1 px-3 py-1.5 bg-[#6C5DD3] hover:bg-[#5b4ec2] text-white text-xs font-medium rounded-md transition shadow-sm">
                                    Edit
                                </button>
                                <button onclick="openDeleteModal('{{ $cat->id }}')" class="flex items-center gap-1 px-3 py-1.5 bg-red-500 hover:bg-red-700 text-white text-xs font-medium rounded-md transition shadow-sm">
                                    Hapus
                                </button>
                            </div>
                        </td>
                    ...
                    @empty
                    <tr><td colspan="3" class="text-center py-8 text-gray-500">Tidak ada data ditemukan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <!-- Pagination -->
        <div class="mt-4">{{ $ticketCategories->links() }}</div>
    </div>
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto flex items-center justify-center p-4">
            <div class="relative transform rounded-xl bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-md border border-gray-200">
                <div class="bg-white px-6 py-6">
                    <h3 class="text-lg font-bold text-[#6C5DD3] mb-4 border-b pb-2">Tambah Kategori Tiket</h3>
                    <form action="{{ route('master.ticket-categories.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Apakah Berbayar?</label>
                            <select name="is_paid" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition">
                                <option value="1">Berbayar (Paid)</option>
    ...
    <div id="editModal" class="relative z-50 hidden" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto flex items-center justify-center p-4">
            <div class="relative transform rounded-xl bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-md border border-gray-200">
                <div class="bg-white px-6 py-6">
                    <h3 class="text-lg font-bold text-[#6C5DD3] mb-4 border-b pb-2">Edit Kategori Tiket</h3>
                    <form id="editForm" method="POST" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Aapakah Berbayar?</label>
                            <select name="is_paid" id="edit_is_paid" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition">
                                <option value="1">Berbayar (Paid)</option>
                                <option value="0">Gratis (Free)</option>
                            </select>
                        </div>
    ...
    <script>
        ...
        function openEditModal(id, name, isPaid) {
            document.getElementById('edit_name').value = name;
            document.getElementById('edit_is_paid').value = isPaid;
            document.getElementById('editForm').action = '/admin/dashboard/master/ticket-categories/' + id;
            document.getElementById('editModal').classList.remove('hidden');
        }

        function openDeleteModal(id) {
            document.getElementById('deleteForm').action = '/admin/dashboard/master/ticket-categories/' + id;
