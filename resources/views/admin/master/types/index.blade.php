...
@section('content')
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-4 px-4 font-bold text-gray-800">{{ $type->name }}</td>
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="openEditModal('{{ $type->id }}', '{{ $type->name }}')" class="flex items-center gap-1 px-3 py-1.5 bg-[#6C5DD3] hover:bg-[#5b4ec2] text-white text-xs font-medium rounded-md transition shadow-sm">
                                    Edit
                                </button>
    ...
    <script>
        ...
        function openEditModal(id, name) {
            ...
            // Pastikan Route Prefixnya benar (admin/master/types)
            document.getElementById('editForm').action = '/admin/dashboard/master/types/' + id;
            document.getElementById('editModal').classList.remove('hidden');
        }

        function openDeleteModal(id) {
            document.getElementById('deleteForm').action = '/admin/dashboard/master/types/' + id;
