...
@section('content')
    <div class="space-y-6">
        ...
        <!-- Jika sukses dan bukan 'dihapus' (karena dihapus pakai modal khusus) -->
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($eventKinds as $kind)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="py-4 px-6 text-gray-500">{{ $loop->iteration + ($eventKinds->currentPage() - 1) * $eventKinds->perPage() }}</td>
                        <td class="py-4 px-4 font-bold text-gray-800">{{ $kind->name }}</td>
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="openEditModal('{{ $kind->id }}', '{{ $kind->name }}')" class="flex items-center gap-1 px-3 py-1.5 bg-[#6C5DD3] hover:bg-[#5b4ec2] text-white text-xs font-medium rounded-md transition shadow-sm">
                                    Edit
                                </button>
            ...
            <div class="px-6 py-4">{{ $eventKinds->links() }}</div>
        </div>
    </div>
    ...
    <script>
        ...
        function openEditModal(id, name) {
            ...
            document.getElementById('editForm').action = '/admin/dashboard/master/event-kinds/' + id ;
            openModal('editModal');
        }

        function openDeleteModal(id) {
            document.getElementById('deleteForm').action = '/admin/dashboard/master/event-kinds/' + id;
        ...
        // Trigger Success Modal jika session ada kata 'dihapus'
        @if(session('success') && str_contains(session('success'), 'dihapus'))
