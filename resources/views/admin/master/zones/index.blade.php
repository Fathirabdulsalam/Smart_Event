...
@section('content')
    <div class="space-y-6">
        ...
        <!-- Tampilkan alert hijau hanya jika bukan pesan hapus (karena hapus pakai modal success) -->
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex justify-between items-center">
            <h2 class="text-xl font-bold text-gray-800">Master Zona Waktu</h2>
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 text-gray-500 text-xs uppercase">
                    <tr>
                        ...
                        <th class="py-4 px-4">Zone Name</th>
                        <th class="py-4 px-4">GMT Offset</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="py-4 px-4 font-bold text-gray-800">{{ $zone->name }}</td>
                        <td class="py-4 px-4 text-gray-600">{{ $zone->gmt_offset }}</td>
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="openEditModal('{{ $zone->id }}', '{{ $zone->name }}', '{{ $zone->gmt_offset }}')" class="flex items-center gap-1 px-3 py-1.5 bg-[#6C5DD3] hover:bg-[#5b4ec2] text-white text-xs font-medium rounded-md transition shadow-sm">
                                    Edit
                                </button>
                                <button onclick="openDeleteModal('{{ $zone->id }}')" class="flex items-center gap-1 px-3 py-1.5 bg-red-500 hover:bg-red-700 text-white text-xs font-medium rounded-md transition shadow-sm">
                                    Delete
                                </button>
                            </div>
                        </td>
                    ...
                    @empty
                    <tr><td colspan="4" class="text-center py-8 text-gray-500">No zones found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto flex items-center justify-center p-4">
            <div class="relative transform rounded-xl bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-md border border-gray-200">
                <div class="bg-white px-6 py-6">
                    <h3 class="text-lg font-bold text-[#6C5DD3] mb-4 border-b pb-2">Add Time Zone</h3>
                    <form action="{{ route('master.zones.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Name</label>
                            <input type="text" name="name" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition" placeholder="e.g. WIB" required>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">GMT Offset</label>
                            <input type="text" name="gmt_offset" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition" placeholder="e.g. +07:00" required>
                        </div>
    ...
    <div id="editModal" class="relative z-50 hidden" role="dialog" aria-modal="true">
        ...
        <div class="fixed inset-0 z-10 overflow-y-auto flex items-center justify-center p-4">
            <div class="relative transform rounded-xl bg-white text-left shadow-xl transition-all sm:w-full sm:max-w-md border border-gray-200">
                <div class="bg-white px-6 py-6">
                    <h3 class="text-lg font-bold text-[#6C5DD3] mb-4 border-b pb-2">Edit Time Zone</h3>
                    <form id="editForm" method="POST" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">GMT Offset</label>
                            <input type="text" name="gmt_offset" id="edit_gmt_offset" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#6C5DD3] outline-none transition" required>
                        </div>
                        <div class="flex justify-end gap-2 mt-6">
    ...
    <script>
        ...
        function openEditModal(id, name, offset) {
            document.getElementById('edit_name').value = name;
            document.getElementById('edit_gmt_offset').value = offset;
            
            // Set URL Action Form
            document.getElementById('editForm').action = '/admin/dashboard/master/zones/' + id;
            
            document.getElementById('editModal').classList.remove('hidden');
        }

        function openDeleteModal(id) {
            document.getElementById('deleteForm').action = '/admin/dashboard/master/zones/' + id;
        ...
        // Error Validation: Reopen Create Modal (kecuali method PUT)
